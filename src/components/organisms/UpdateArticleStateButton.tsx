...
export const UpdateArticleStateButton = ({
    ...
    const { mutateAsync, isLoading } = trpcClient.articles.update.useMutation({
        onSuccess() {
            toast.success(`Article ${articleId} is ${!published ? 'published' : 'unpublished'}`, {
    ...
    return (
        <Button
            ...
            size={'sm'}
            loading={isLoading}
            className={cn(className, 'bg-slate-800 no-underline group cursor-pointer relative shadow-2xl shadow-zinc-900 rounded-full p-px text-xs font-semibold leading-6  text-white inline-block min-w-fit w-1/3')}
            onClick={async () => {
